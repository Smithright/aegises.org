...
const phases          = [
 {title:'One dictation. Two durable commitments.',detail:'Agentic intake interprets the dictate, frames a spike and a monthly-review edict, and proposes both. The owning services check authority and persist the records.',ticket:'Frame → admit → save',code:'Awaiting research and context',gate:'Admission required'},
 {title:'Bind the right knowledge and rules.',detail:'The agent investigates the bill and selects relevant policies, templates and examples. Governed services validate and record the proposed bindings.',ticket:'Research → implementation',code:'Sources and requirements linked',gate:'Rules remain separately governed'},
 ...
 {title:'Produce a control candidate.',detail:'The agent designs a control service and proposes its implementation, tests and requirement mappings. News feeds a sourced AI Compliance News Report; findings can create linked follow-up tickets.',ticket:'Candidate → review',code:'Control + tests + source mapping',gate:'Candidate awaits admission'},
...
const notes                                 = {
 chat:['Agentic intake','The agent interprets the original words and proposes tickets, edicts, an update to existing work, or no new ticket. The service independently validates and records the permitted disposition.'],
 ...
 code:['Control service candidate','This component exposes scoped suspend and resume operations with policy enforcement and attributable results. Source code, tests and release artifacts implement it; no language or file format defines the component. The proposed compliance example is not a qualified implementation.'],
...
const q =                        (s        )    => { const el = document.querySelector   (s); if (!el) throw new Error(`Missing ${s}`); return el; };
const factory=q             ('#factory'), play=q                   ('#play'), progress=q             ('#progress');
...
function jump(next       )      {stage=Math.max(0,Math.min(5,next));elapsed=0;running=false;render();updateProgress();}
function updateProgress()      {progress.style.width=`${((stage+(stage===5?1:elapsed/duration))/6)*100}%`;}
function tick(now       )      {
...
q                   ('#speed').addEventListener('change',event=>{speed=Number((event.target                     ).value);});
q                  ('#denied').addEventListener('change',event=>{denied=(event.target                    ).checked;jump(4);});
const inspection=q('#inspection');let lastInspected             =null;
function inspect(key       ,origin        )      {const note=notes[key];if(!note)return;running=false;render();q('#inspect-title').textContent=note[0];q('#inspect-body').textContent=note[1];inspection.hidden=false;lastInspected=origin;q                   ('#close-inspection').focus();}
document.querySelectorAll            ('[data-inspect]').forEach(el=>{el.addEventListener('click',()=>inspect(el.dataset.inspect ,el));el.addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();inspect(el.dataset.inspect ,el);}});});
function closeInspection()      {inspection.hidden=true;(lastInspected                   )?.focus();}
...
document.querySelectorAll             ('[data-jump]').forEach(el=>el.addEventListener('click',()=>jump(Number(el.dataset.jump))));
// Print the explanatory content even when it is normally disclosed on demand.
let printOpen                     =[];
window.addEventListener('beforeprint',()=>{printOpen=Array.from(document.querySelectorAll                    ('main details:not([open]):not(.origin)'));printOpen.forEach(el=>el.open=true);});
...
const concurrency=document.querySelector             ('.concurrency-demo');
if(concurrency){
 const buttons=Array.from(concurrency.querySelectorAll                   ('button[data-moment]'));
