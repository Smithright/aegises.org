# AGI Reactor · Draft 2.1 · 14 September 2026
...
A Safe, Governable Reference Architecture for Enterprise AGI
...
Language becomes commitments. Definitions compose execution.

Swipe the diagram to follow the system →

The Reactor: one system, two kinds of commitmentHuman intent becomes tickets and edicts. Tickets retain work. Edicts inform versioned definitions and policy. Typed bindings compose skills and agent profiles; the context compositor assembles a frame for an agent. Governed services produce delivery. Reactions and routines continue the loop. NATS connects the services and owned objects preserve state.
...
Tickets preserve work. Edicts preserve direction. Typed bindings connect reusable definitions to agents; governed services turn their actions into results.Follow one mandate ↓

Example implementation · proposed

## A mandate becomes a system.

Watch the factory. Select any component to inspect its role.

WorkAuthorityEvidence
...
Illustration only · no model calls, meetings or deployments.Download the example output ↓

Inside the system

## Follow the work. Keep the meaning.

Reference diagrams explain the contract. Blue example labels show one proposed implementation of it.
01
...
A dictate is retained as a source. Accepted work becomes a ticket; enduring direction becomes an edict. Every delivered artifact keeps its lineage.From dictation to delivered artifactsAn attributed dictate creates a spike ticket and a standing edict. The ticket produces versioned control candidates. Qualification and admission precede delivery. Delivery evidence returns to the ticket. The edict continues through monthly reviews and linked work.
SOURCE
OBLIGATION
ARTIFACT
DELIVERY
...
Originals remain. Revisions accumulate. “Done” requires the ticket’s acceptance evidence.
...
Definitions describe reusable behavior. Identities own work. Versioned policy bindings govern what those identities may do to typed objects.The typed object model: definitions, identity, work and authorityEdicts inform skills and may lead to policies only through authorized review and adoption. Templates and examples support skills. Agent types use skills and are instantiated by agent identities. A compositor resolves the profile and permitted sources into a cognitive frame consumed by an attempt. Tickets have zero or one assignee, many historical attempts and at most one current attempt. Artifacts and evidence link to tickets. Attempts request operations and retain receipts. Versioned policy bindings apply to agent types, service methods, assignment, status transitions and resources; type membership alone grants no permission.
REUSABLE DEFINITIONS
COMPOSITION
WORKING CONTEXT
Edict
...
Monthly-review edict · compliance-analysis skill · report template · assurance examples · leadership agent profile · source-bound cognitive frame.
...
A binding is a typed link to a policy revision and target scope. Policies remain rules in the shared decision model; these scopes do not require six new services. Policy promotion has its own authorized review and activation path.
04
...
The ontology gives a call its meaning. Local policy decides admission; the owning service enforces it at use. A permit is not a completed effect.A typed request crosses a governed service boundaryThe ontology resolves a request to a typed service method. Local policy bindings and verified facts determine admission. The owning service enforces authority and resource concurrency at use. A result or unknown outcome returns under the same operation identity and links to the ticket. NATS transports calls; it does not grant authority.
SHARED MEANING
LOCAL AUTHORITY
...
A sourced report reaches the right leaders each month. Decisions create owned follow-up work. That work informs the next review.Follow the continuing loop ↗The edict keeps a monthly review aliveSourced AI compliance news becomes a report. Proposed legal, security, engineering and accountable AI leadership roles review it monthly. Decisions create owned follow-up tickets. The routine continues under its standing edict.
...
Proposed roles shown. Actual participants, ownership and schedule require adoption under the edict.
...
Anderson’s reference monitor mediates protected access. Its implementation must resist tampering, be invoked for every protected reference and permit thorough analysis. [1] Saltzer and Schroeder sharpen the obligation: explicit permission, complete mediation and least privilege. [2]
...
ObjectA stable identity, type, revision and owner. Its storage representation can change.
...
Message / invokeAttributed communication; a typed request through an authenticated service contract.

Edict / policyEnduring direction; enforceable rules require authorized promotion, versioning and activation.

Skill / agent typeReusable procedure and composed profile. Neither is an identity or permission grant.

Compositor / framePermitted context assembled with intent, constraints, sources and pending effects.

Reaction / routineAn event or schedule invokes governed work. The monthly review continues under its adopted edict.

Operation / evidenceImmutable effect intent and attributable outcome. Original caller, target and arguments survive takeover.
...
The reference stack and its evidence
...
Smithright, Ryan. AGI Reactor — A Safe, Governable Reference Architecture for Enterprise AGI. AEGISES, draft 2.1, 14 September 2026. Version permalink · BibTeX · CSL-JSON.
...
AGI Reactor · Draft 2.1 · 14 September 2026Previous edition ↗Back to the beginning ↑
